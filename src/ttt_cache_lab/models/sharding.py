...
def build_model_shard_plan(config: Any, *, device_type: str, device_ids: list[int]) -> ModelShardPlan:
    ...
    reserve_first_device = (
        ...
        and len(devices) > 1
        and model_type
        in {"llama", "mistral", "qwen2", "qwen2_moe", "qwen3", "gemma", "gemma2", "gemma3", "gemma3_text"}
    )
    if reserve_first_device:
        weights = [0.75, *([1.0] * (len(devices) - 1))]
        raw = [num_layers * weight / sum(weights) for weight in weights]
        quotas = [max(1, int(value)) for value in raw]
        while sum(quotas) > num_layers:
            index = max(range(len(quotas)), key=lambda i: (quotas[i] - raw[i], quotas[i], -i))
            if quotas[index] <= 1:
                break
            quotas[index] -= 1
        while sum(quotas) < num_layers:
            index = max(range(len(quotas)), key=lambda i: (raw[i] - quotas[i], i))
            quotas[index] += 1
        expanded = [device for device, quota in zip(devices, quotas, strict=True) for _ in range(quota)]
