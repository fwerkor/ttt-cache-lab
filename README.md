...
- a default 47-configuration, 141-job core manifest plus an archived 84-configuration extended manifest, spanning 1.5B, 3B, 4B, 7B, and 32B evidence with dense/sliding-window/local-global/MoE architecture transfer and a 7B code model;
...
└── 当前状态
    ...
    │       └── 无 run_failure.json 或 .failed
    ├── 1.5B common_words
    │   ├── seed 7：[运行；已写入 24,624 / 64,512 条记录，无当前失败标记]
    │   └── seed 17、29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B 三个核心任务：[待做]
    ├── 32B 三个核心任务：[暂停]
...
这些实验目前不计入核心冻结的 141 个正式 seed-run，但决定 Planner 是否有足够强的机制依据。
...
全部论文实验
│
├── 核心冻结矩阵：47 configs / 141 seeds
│   │
│   ├── E1：1 config / 3 seeds；完整配置 1/1
│   ├── E2：3 configs / 9 seeds；完成 1，运行 1
...
│
├── 扩展可选矩阵：84 configs / 252 seeds
│   └── configs/paper/study_extended.yaml；仅在核心证据完成且算力有余时运行
│
└── W/B 探索线
    ├── W1/W2/W3：已有部分原始运行，缺分析闭环
...
当前最准确的概括是：**实验设计和代码框架已经相当完整，论文正式数据已完成首个三-seed 冻结配置。核心矩阵严格口径下已有 1/47 个配置完成；单 seed 层面已正式验收 6 个，另有 2 个正在运行。**
...
The frozen protocol is in [`docs/paper_experiment_protocol.md`](docs/paper_experiment_protocol.md). The default core matrix contains 47 configurations and expands to 141 jobs over seeds 7, 17, and 29. The original 84-configuration matrix remains available as `configs/paper/study_extended.yaml`. Qwen2.5-7B remains the complete main evaluation, Qwen2.5-32B supplies large-model headline evidence, and A1 retains lightweight dense/sliding-window/local-global/MoE transfer screening.
