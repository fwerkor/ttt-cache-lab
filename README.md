...
- 严格完成配置：**8/51**（核心 5/47；W-F 3/4）
- 已正式验收的单 seed：**28/153**（核心 18/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation 与 common_words 的 seed 7、17、29、multi_hop_tracing seed 7、17，以及 Qwen2.5-7B aggregation seed 7；前两个配置均已完成三 seed 验收
...
- 当前正在运行：
  ...
  - W4 Qwen2.5-1.5B seed 29：8,874 / 11,136 条 blockwise records
- 当前失败且不得计入正式结果：
...
└── 当前状态
    ...
    │   │   ├── `version_summary.csv` 共 336 条数据行；`.success` 返回码为 0
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   └── seed 29：[运行；已生成 `run_metadata.json`，主结果与汇总尚未生成]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B aggregation
    │   ├── seed 7：[完成]
    │   │   ├── `.success`、`run_metadata.json`、`records.jsonl`、`summary.csv`、`version_summary.csv` 均完整且非空
    ...
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   └── seed 17/29：[待做]
    ├── 7B multi_hop_tracing seed 7：[运行；已生成 `run_metadata.json`，主结果与汇总尚未生成]
    ├── 7B 其他核心任务：[待做]
    ├── 32B 三个核心任务：[暂停]
    └── 正式完成度：2/12 配置，9/36 seed
```
...
└── 当前状态
    ...
    ├── seed 7：[完成][验收]
    ├── seed 17：[部分；不得计入正式结果]
    ├── seed 29：[运行，6,554 / 11,136 条 blockwise records；`.success` 与最终汇总尚未生成]
...
│   ├── E3：12 configs / 36 seeds；完成 2，运行 2
...
│   ├── W2：3/3 seeds；[完成][冻结]
│   ├── W3：3/3 seeds；[完成][冻结]
│   └── W4/B1：1/3 seeds；seed 17 部分，seed 29 运行中
│
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 8/51 个完整配置并验收 28/153 个 seed-run。W1-W3 已闭环，W4 只剩两个 seed；后续方法探索集中在 B，E2/E3 可继续并行运行。**
