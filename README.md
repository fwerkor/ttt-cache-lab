...
- 当前运行或待补齐验收条件：
  ...
  - Table 1 joint-LoRA held-out v1：controlled seed 7 与 seed 17 正在运行，分别完成 4/32 与 3/32 个样本；当前分片样本均有有效 `run_metadata.json` 和非空主结果/汇总产物，但 seed-run 尚无 `.success`，不得标记完成。HotpotQA seed 7、17、29 分别仅完成 1、0、1 个样本后发生 NPU OOM，2WikiMQA seed 7、17 均在首个样本发生 NPU OOM；这些运行保留未解决的 `run_failure.json`，无 `.success`，必须无 OOM 恢复或重跑并完成全部 32 个样本后方可验收。2WikiMQA seed 29 尚未产生正式运行产物
