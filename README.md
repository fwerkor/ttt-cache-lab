...
- 当前运行或待补齐验收条件：
  - E3 1.5B multi_needle seed 29：task probe 与 `run_metadata.json` 已生成，当前完成 976/1,344（约 72.6%），尚无 `.success` 与最终汇总产物
  - E3 7B common_words seed 7：task probe 与 `run_metadata.json` 已生成，当前完成 281/512（约 54.9%），尚无 `.success` 与最终汇总产物
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 9/51 个完整配置并验收 33/153 个 seed-run。W1-W3 已闭环；W4 seed 17/29 均已有完整 blockwise 结果，但仍缺 `run_metadata.json`，不满足正式验收条件；后续方法探索集中在 B，E2/E3 可继续并行运行。**
