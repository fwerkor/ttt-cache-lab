...
- 已正式验收的单 seed：**31/153**（核心 21/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation、common_words 与 multi_hop_tracing 的 seed 7、17、29，Qwen2.5-1.5B multi_needle seed 7，以及 Qwen2.5-7B aggregation seed 7、multi_hop_tracing seed 7；前三个配置均已完成三 seed 验收
...
- 当前正在运行：
  - E3 1.5B multi_needle seed 17：task probe 与 `run_metadata.json` 已生成，当前完成 1,068/1,344（约 79.5%），尚无 `.success` 与最终汇总产物
  - E3 7B multi_needle seed 7：task probe 与 `run_metadata.json` 已生成，当前完成 239/512（约 46.7%），尚无 `.success` 与最终汇总产物
  - E3 7B common_words seed 7：task probe 与 `run_metadata.json` 已生成，当前完成 112/512（约 21.9%），尚无 `.success` 与最终汇总产物
  - W4 seed 17：重新运行中，已写出 5,626/11,136 条 blockwise records（约 50.5%）；当前尚无 `.success` 与 `run_metadata.json`，不得计入正式结果
- 当前失败且不得计入正式结果：
...
└── 当前状态
    ...
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   ├── seed 17：[运行中；1,068/1,344，约 79.5%；不得计入正式结果]
    │   └── seed 29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B aggregation
    │   ├── seed 7：[完成]
...
└── 当前状态
    ...
    ├── seed 7：[完成][验收]
    ├── seed 17：[运行中；5,626/11,136 条 blockwise records，约 50.5%；不得计入正式结果]
...
│   ├── E3：12 configs / 36 seeds；完成 3，已验收 12/36 seeds，当前运行 3
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 9/51 个完整配置并验收 31/153 个 seed-run。W1-W3 已闭环；W4 seed 17/29 均仍不满足正式验收条件；后续方法探索集中在 B，E2/E3 可继续并行运行。**
