...
- 当前运行或待补齐验收条件：
  - E2 real-task extension：HotpotQA seed 7 与 2WikiMQA seed 7 均已完成并通过产物验收；两项均有 `.success`、有效 `run_metadata.json`、448 条有效 `records.jsonl`、448 行数据的 `summary.csv`、14 行数据的 `version_summary.csv`，task probe 分别为 0.0522 与 0.0850 且满足质量阈值，未发现 `run_failure.json` 或 `.failed`。该扩展不属于 51 配置 / 153 seed-run 的投稿必需冻结矩阵，因此不增加正式验收总数；对应 seed 17 已启动，当前均完成 30/32 个 target（93.8%），已有有效 `run_metadata.json`、420 条增量 `records.jsonl` 与 420 行数据的 `summary.csv`，未发现 `run_failure.json` 或 `.failed`；仍缺 `.success` 和最终 `version_summary.csv`，不得标记完成
