...
- 已正式验收的单 seed：**29/153**（核心 19/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation 与 common_words 的 seed 7、17、29、multi_hop_tracing seed 7、17，以及 Qwen2.5-7B aggregation 与 multi_hop_tracing seed 7；前两个配置均已完成三 seed 验收
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 8/51 个完整配置并验收 29/153 个 seed-run。W1-W3 已闭环，W4 只剩两个 seed；后续方法探索集中在 B，E2/E3 可继续并行运行。**
