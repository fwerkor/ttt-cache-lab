...
- 当前运行或待补齐验收条件：
  - Table 1 joint-LoRA calibration v1：controlled、HotpotQA、2WikiMQA 的 seed 53 与 seed 61 共 6 个运行均已汇总到 24/24 targets，并生成有效 `run_metadata.json`、576 条 `records.jsonl`、576 行数据的 `summary.csv` 和 72 行数据的 `version_summary.csv`；但均无 `.success`，且保留未解决的 `run_failure.json`（NPU OOM，单次额外分配 592 MiB 失败）。当前无对应进程，必须清除失败状态并完成无 OOM 的正式重跑或恢复验收后方可标记完成；现阶段不得计入正式结果
