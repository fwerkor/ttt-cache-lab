...
- 当前运行或待补齐验收条件：
  ...
  - Table 1 joint-LoRA held-out v1：single-NPU memory-fix 队列仍在运行，尚无完整 seed-run 通过验收。controlled seed 7 的首个隔离样本在完成 2/3 targets 后再次 NPU OOM，并保留未解决的 `run_failure.json`；seed 17 的首个样本已启动但尚未形成完整结果，seed 29 尚无产物。HotpotQA seed 7 与 seed 29、2WikiMQA seed 7/17/29 均已完整完成 7 个隔离样本，第 8 个样本正在运行或仅部分完成；对应 chunk 当前各汇总 315 条 `records.jsonl` 和 315 行数据的 `summary.csv`，但均无 seed-run 级 `.success` 与 `run_metadata.json`。HotpotQA seed 17 仍仅启动首个样本，尚无完整汇总。所有任务必须完成全部 32 个样本、生成 seed-run 级 `.success`、有效 `run_metadata.json`、主结果与最终汇总文件，且不存在未解决的 `run_failure.json`，方可标记完成
