...
- 当前运行或待补齐验收条件：
  ...
  - Table 1 joint-LoRA held-out v1：single-NPU memory-fix 队列仍在运行，尚无完整 seed-run 通过验收。controlled seed 7 的首个隔离样本在完成 2/3 targets 后再次 NPU OOM，并保留未解决的 `run_failure.json`；seed 17 与 seed 29 均已完整完成 2 个隔离样本，后续样本正在运行或仅部分完成。HotpotQA seed 7 与 seed 29 均已完整完成 9 个隔离样本，第 10 个样本正在运行或仅部分完成；各 chunk 合计汇总 405 条 `records.jsonl` 和 405 行数据的 `summary.csv`。HotpotQA seed 17 已完整完成 1 个隔离样本，另有样本正在运行或仅部分完成。2WikiMQA seed 7/17/29 均已完整完成 8 个隔离样本，对应 chunk 各汇总 360 条 `records.jsonl` 和 360 行数据的 `summary.csv`，队列已转入后续任务。上述任务均无 seed-run 级 `.success` 与 `run_metadata.json`，不得标记完成；必须完成全部 32 个样本、生成 seed-run 级 `.success`、有效 `run_metadata.json`、主结果与最终汇总文件，且不存在未解决的 `run_failure.json`，方可通过验收
