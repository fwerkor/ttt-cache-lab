...
- 当前运行或待补齐验收条件：
  - Table 1 joint-LoRA calibration v1：此前 controlled、HotpotQA、2WikiMQA 的 seed 53 与 seed 61 共 6 个运行虽已汇总到 24/24 targets，但均无 `.success` 且保留未解决的 NPU OOM `run_failure.json`，不得计入正式结果。2026-07-24 启动的 isolated-sample memory-fix 正式重跑仍在进行：controlled seed 53/61 各已完整完成 5/8 个隔离样本，第 6 个样本正在运行；每个已完成样本均有有效 `run_metadata.json`，并生成 192 条有效 `records.jsonl`、192 行 `summary.csv` 和 192 行 `version_summary.csv`。HotpotQA 与 2WikiMQA 的 seed 53/61 各已完整完成 2/8 个隔离样本，第 3 个样本正在运行。当前未发现新的 `run_failure.json`，但 6 个 seed-run 均尚无 seed-run 级 `.success` 与 `run_metadata.json`，不得标记完成。必须完成全部 8 个正式样本并生成 seed-run 级完整验收产物，且不存在未解决的失败文件，方可通过验收
