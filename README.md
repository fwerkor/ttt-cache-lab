...
- 当前正在运行：
  - E2 Qwen2.5-7B LongBench v2 seed 29：6,552 / 8,640 条
  - E3 1.5B multi_hop_tracing seed 17：14,664 / 32,256 条
...
└── 当前状态
    ...
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   └── seed 29：[运行，6,552 / 8,640 条；`.success` 与 `version_summary.csv` 尚未生成]
...
└── 当前状态
    ...
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   ├── seed 17：[运行，14,664 / 32,256 条；`.success` 与 `version_summary.csv` 尚未生成]
    │   └── seed 29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B 三个核心任务：[待做]
    ├── 32B 三个核心任务：[暂停]
