...
- 已正式验收的单 seed：**27/153**（核心 17/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation 与 common_words 的 seed 7、17、29，以及 multi_hop_tracing seed 7、17；前两个配置均已完成三 seed 验收
...
└── 当前状态
    ...
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B 三个核心任务：[运行；aggregation seed 7 与 multi_hop_tracing seed 7 已启动，当前仅生成 `run_metadata.json`，尚无主结果或汇总文件]
    ├── 32B 三个核心任务：[暂停]
    └── 正式完成度：2/12 配置，8/36 seed
```
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 8/51 个完整配置并验收 27/153 个 seed-run。W1-W3 已闭环，W4 只剩两个 seed；后续方法探索集中在 B，E2/E3 可继续并行运行。**
