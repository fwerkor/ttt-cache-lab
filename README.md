...
- 当前运行或待补齐验收条件：
  ...
  - Table 1 joint-LoRA held-out v1：controlled seed 7 与 seed 17 的 chunk 0 均已完成 8/8 个样本并生成有效 `run_metadata.json`、主结果和汇总文件，但完整 seed-run 仅完成 8/32，尚无 `.success`；当前队列进程已停止，剩余各 24 个样本，按部分完成处理，不得标记完成。HotpotQA seed 7 已完整完成 1 个样本，随后两个样本发生 NPU OOM；seed 17 的两个已启动样本均 OOM；seed 29 已完整完成 1 个样本，随后一个样本 OOM。2WikiMQA seed 7 与 seed 17 均在首个样本发生 NPU OOM；上述失败样本均保留未解决的 `run_failure.json`，所有 seed-run 均无 `.success`，必须无 OOM 恢复或重跑并完成全部 32 个样本后方可验收。controlled seed 29 与 2WikiMQA seed 29 尚未产生正式运行产物
