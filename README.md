...
- 当前运行或待补齐验收条件：
  - E3 1.5B multi_needle seed 29：task probe 与 `run_metadata.json` 已生成，当前完成 786/1,344（约 58.5%），尚无 `.success` 与最终汇总产物
  - E3 7B common_words seed 7：task probe 与 `run_metadata.json` 已生成，当前完成 254/512（约 49.6%），尚无 `.success` 与最终汇总产物
