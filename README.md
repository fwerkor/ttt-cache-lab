...
- 当前运行或待补齐验收条件：
  - Table 1 joint-LoRA calibration v1：此前 controlled、HotpotQA、2WikiMQA 的 seed 53 与 seed 61 共 6 个运行虽已汇总到 24/24 targets，但均无 `.success` 且保留未解决的 NPU OOM `run_failure.json`，不得计入正式结果。2026-07-24 已启动 isolated-sample memory-fix 正式重跑，6 个 seed-run 均已创建 sample 0000，并生成有效的样本级 `run_metadata.json`；当前各为 0/1 target，均在运行中，尚无样本级或 seed-run 级 `.success`、主结果与最终汇总文件，也尚未产生新的 `run_failure.json`。必须完成全部正式样本并生成 seed-run 级完整验收产物，且不存在未解决的失败文件，方可标记完成
