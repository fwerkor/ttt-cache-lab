...
- 已正式验收的单 seed：**30/153**（核心 20/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation、common_words 与 multi_hop_tracing 的 seed 7、17、29，以及 Qwen2.5-7B aggregation 与 multi_hop_tracing seed 7；前三个配置均已完成三 seed 验收
  - W1/W2/W3：seed 7、17、29 均已验收
  - W4：seed 7 已验收；seed 17 为未完成目录；seed 29 已运行结束但缺少 `run_metadata.json`，暂不得验收
- 当前正在运行：
  - E3 1.5B multi_needle seed 7：task probe 已完成，mean score 0.96875（31/32 非零），`run_metadata.json` 有效；进程已运行超过 10 小时但未写出主结果，且未出现在 NPU 进程表中，按疑似卡死处理，暂不得验收
- 当前部分完成且不得计入正式结果：
...
└── 当前状态
    ...
    │   │   ├── task probe mean score 0.96875（31/32 非零），`run_metadata.json` 有效
    │   │   ├── 进程运行超过 10 小时仍未生成 `records.jsonl`、`summary.csv`、`version_summary.csv` 或 `.success`
...
└── 当前状态
    ...
    ├── seed 7：[完成][验收]
    ├── seed 17：[部分；不得计入正式结果]
    ├── seed 29：[部分；`.success` 返回码为 0，11,136 条 blockwise records 与最终汇总已生成，无失败标记；缺少验收必需的 `run_metadata.json`，不得计入正式结果]
    └── 整体：1/3 seed；[冻结][待补齐元数据]
```
...
│   ├── E3：12 configs / 36 seeds；完成 3，运行 1（当前运行疑似卡死）
...
│   ├── W2：3/3 seeds；[完成][冻结]
│   ├── W3：3/3 seeds；[完成][冻结]
│   └── W4/B1：1/3 seeds；seed 17 部分，seed 29 已运行结束但缺少 `run_metadata.json`，不得验收
│
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 9/51 个完整配置并验收 30/153 个 seed-run。W1-W3 已闭环；W4 seed 17/29 均仍不满足正式验收条件；E3 当前运行的 multi_needle seed 7 疑似卡死，后续方法探索集中在 B。**
