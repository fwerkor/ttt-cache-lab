...
- 当前运行或待补齐验收条件：
  - E3 7B common_words seed 7：task probe 与 `run_metadata.json` 已生成，当前完成 394/512（约 77.0%），尚无 `.success` 与最终汇总产物
  - E3 1.5B needle_absent seed 7：task probe 与 `run_metadata.json` 已生成，当前完成 667/1,344（约 49.6%），尚无 `.success` 与最终汇总产物
...
└── 当前状态
    ...
    │       └── 无 `run_failure.json` 或 `.failed`
    ├── 1.5B needle_absent
    │   ├── seed 7：[运行中；380/1,344，约 28.3%；不得计入正式结果]
    │   └── seed 17/29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B aggregation
    │   ├── seed 7：[完成]
...
│   ├── E3：12 configs / 36 seeds；完成 3，已验收 14/36 seeds，当前运行 2
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 10/51 个完整配置并验收 34/153 个 seed-run。W1-W3 已闭环；W4 seed 17/29 均已有完整 blockwise 结果，但仍缺 `run_metadata.json`，不满足正式验收条件；后续方法探索集中在 B，E2/E3 可继续并行运行。**
