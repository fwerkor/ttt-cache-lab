...
└── 当前状态
    ...
    │   │   ├── .success 返回码为 0
    │   │   └── 无 run_failure.json 或 .failed
    │   └── seed 29：[运行；已写入 6,489 / 10,752 条完整记录，无当前失败标记；尚缺 `.success` 与 `version_summary.csv`；旧 OOM 结果不计论文数据]
...
└── 当前状态
    ...
    │   │   ├── `.success` 返回码为 0
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   ├── seed 17：[运行；已写入 18,528 / 32,256 条完整记录，无当前失败标记；尚缺 `.success` 与 `version_summary.csv`]
    │   └── seed 29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B 三个核心任务：[待做]
    ├── 32B 三个核心任务：[暂停]
