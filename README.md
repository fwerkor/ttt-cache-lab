...
- 已正式验收的单 seed：**36/153**（核心 26/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation、common_words、multi_hop_tracing 与 multi_needle 的 seed 7、17、29，Qwen2.5-1.5B needle_absent seed 7，以及 Qwen2.5-7B aggregation、multi_hop_tracing、multi_needle 与 common_words seed 7；前四个配置均已完成三 seed 验收
...
- 当前运行或待补齐验收条件：
  - E3 1.5B needle_absent seed 17：task probe 与 `run_metadata.json` 已生成，当前完成 1,128/1,344（约 83.9%），尚无 `.success` 与最终汇总产物
...
└── 当前状态
    ...
    │   │   ├── `.success` 返回码为 0
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   ├── seed 17：[运行中；802/1,344，约 59.7%；不得计入正式结果]
    │   └── seed 29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B aggregation
    │   ├── seed 7：[完成]
...
│   ├── E3：12 configs / 36 seeds；完成 3，已验收 16/36 seeds，当前运行 1
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 10/51 个完整配置并验收 36/153 个 seed-run。W1-W3 已闭环；W4 seed 17/29 均已有完整 blockwise 结果，但仍缺 `run_metadata.json`，不满足正式验收条件；后续方法探索集中在 B，E2/E3 可继续并行运行。**
