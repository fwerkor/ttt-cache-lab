...
- 当前正在运行：
  - E2 Qwen2.5-7B LongBench v2 seed 17：6,912 / 8,640 条
  - E3 1.5B multi_hop_tracing seed 7：12,120 / 32,256 条
...
└── 当前状态
    ...
    │   │   ├── `.success` 返回码为 0
    │   │   └── 无 `run_failure.json` 或 `.failed`
    │   └── seed 17：[运行，6,912 / 8,640 条；`.success` 与 `version_summary.csv` 尚未生成]
...
└── 当前状态
    ...
    │       ├── `.success` 返回码为 0
    │       └── 无 `run_failure.json` 或 `.failed`
    ├── 1.5B multi_hop_tracing：seed 7 [运行，12,120 / 32,256 条；`.success` 与 `version_summary.csv` 尚未生成]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B 三个核心任务：[待做]
    ├── 32B 三个核心任务：[暂停]
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 6/51 个完整配置并验收 22/153 个 seed-run。W1-W3 已闭环，W4 只剩两个 seed；后续方法探索集中在 B，E2/E3 可继续并行运行。**
