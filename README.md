...
- 已正式验收的单 seed：**18/153**（核心 8/141；W-F 10/12）
...
└── 当前状态
    ...
    │   │   ├── .success 返回码为 0
    │   │   └── 无 run_failure.json 或 .failed
    │   └── seed 29：[运行；已写入 1,239 / 10,752 条记录，无当前失败标记；尚缺 `.success` 与 `version_summary.csv`；旧 OOM 结果不计论文数据]
...
└── 当前状态
    ...
    │       └── 无 run_failure.json 或 .failed
    ├── 1.5B common_words
    │   ├── seed 7：[运行；已写入 31,728 / 64,512 条记录，无当前失败标记；尚缺 `.success` 与 `version_summary.csv`]
    │   └── seed 17、29：[待做]
    ├── 其他 1.5B 核心任务：[待做]
    ├── 7B 三个核心任务：[待做]
    ├── 32B 三个核心任务：[暂停]
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 5/51 个完整配置并验收 18/153 个 seed-run。W1-W3 已闭环，W4 只剩两个 seed；后续方法探索集中在 B，E2/E3 可继续并行运行。**
