...
- 已正式验收的单 seed：**37/153**（核心 27/141；W-F 10/12）
  - E1：seed 7、17、29；该配置三 seed 已全部验收
  - E2：Qwen2.5-7B controlled 与 LongBench v2 的 seed 7、17、29；两个配置均已完成三 seed 验收
  - E3：Qwen2.5-1.5B aggregation、common_words、multi_hop_tracing 与 multi_needle 的 seed 7、17、29，Qwen2.5-1.5B needle_absent seed 7、17，以及 Qwen2.5-7B aggregation、multi_hop_tracing、multi_needle 与 common_words seed 7；前四个配置均已完成三 seed 验收
...
- 当前运行或待补齐验收条件：
  - Table 1 joint-LoRA calibration v1：此前 controlled、HotpotQA、2WikiMQA 的 seed 53 与 seed 61 共 6 个运行虽已汇总到 24/24 targets，但均无 `.success` 且保留未解决的 NPU OOM `run_failure.json`，不得计入正式结果。2026-07-24 已启动 isolated-sample memory-fix 正式重跑：controlled seed 53 与 seed 61 的 sample 0000 均已完成 1/1 target，并分别生成 192 条有效 `records.jsonl`、192 行 `summary.csv` 和 192 行 `version_summary.csv`；sample 0001 已启动，当前为 0/1 target。HotpotQA 与 2WikiMQA 的 seed 53/61 均已启动 sample 0000，当前为 0/1 target。6 个 seed-run 均仍在运行，当前仅有样本级有效 `run_metadata.json`，尚无 seed-run 级 `.success` 与 `run_metadata.json`，也未发现新的 `run_failure.json`；不得标记完成。必须完成全部 8 个正式样本并生成 seed-run 级完整验收产物，且不存在未解决的失败文件，方可通过验收
  - Table 1 joint-LoRA held-out v1：single-NPU memory-fix 队列当前已停止，尚无完整 seed-run 通过验收。controlled seed 7 的首个隔离样本在完成 2/3 targets 后 NPU OOM，并保留未解决的 `run_failure.json`；seed 17 与 seed 29 各完整完成 2 个隔离样本，随后样本仅完成 1/3 targets 后中断。HotpotQA seed 7 与 seed 29 各完整完成 9 个隔离样本，第 10 个样本仅完成 1/3 targets 后中断；各 seed 当前累计 420 条 `records.jsonl` 和 420 行数据的 `summary.csv`。HotpotQA seed 17 完整完成 1 个隔离样本，下一样本仅完成 1/3 targets 后中断。2WikiMQA seed 7/17/29 各完整完成 8 个隔离样本，当前各累计 360 条 `records.jsonl` 和 360 行数据的 `summary.csv`。当前无对应实验进程；上述任务均无 seed-run 级 `.success` 与 `run_metadata.json`，不得标记完成。必须完成全部 32 个样本、生成 seed-run 级 `.success`、有效 `run_metadata.json`、主结果与最终汇总文件，且不存在未解决的 `run_failure.json`，方可通过验收
...
└── 当前状态
    ...
    ├── seed 7：[完成][验收]
    ├── seed 17：[部分；不得计入正式结果；`.success` 与 11,136 条 blockwise records 及完整专用汇总均存在，但缺少 `run_metadata.json`]
    ├── seed 29：[部分；不得计入正式结果；`.success` 与 11,136 条 blockwise records 及完整专用汇总均存在，但缺少 `run_metadata.json`]
...
│   ├── E3：12 configs / 36 seeds；完成 3，已验收 17/36 seeds，当前运行 1
...
│   └── A1：12 configs / 36 seeds；Gemma-3-4B 两个 seed 7 因 task probe=0.000<0.050 失败，当前暂停
│
├── W-F 机制冻结矩阵：4 configs / 12 seed-runs
│   ├── W1：3/3 seeds；[完成][冻结]
│   ├── W2：3/3 seeds；[完成][冻结]
│   ├── W3：3/3 seeds；[完成][冻结]
│   └── W4/B1：1/3 seeds；seed 17 部分，seed 29 缺 `run_metadata.json`，均不得计入正式结果
│
...
当前最准确的概括是：**投稿必需证据现为 51 个冻结配置、153 个 seed-run；已完成 10/51 个完整配置并验收 37/153 个 seed-run。W1-W3 已闭环；W4 seed 17/29 均已有完整 blockwise 结果，但仍缺 `run_metadata.json`，不满足正式验收条件；后续方法探索集中在 B，E2/E3 可继续并行运行。**
